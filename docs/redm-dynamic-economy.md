...
この差額は消滅させず、**税として国庫に記帳する**（§10.1）。プレイヤーから見た金額は同じで、
「消えた」が「国庫に入った」に変わるだけ。貯まった税は組合への補助金の原資になる（§10.4）。
...
| 9 | 税 | 買取 `× (1 - tax_sell)` / 販売 `× (1 + tax_buy)` | §10.1 |
...
Config.Census = {
  ...
  daily          = true,
  include        = { 'cash', 'bank' },   -- 集計に含める財布（§14 の確認事項）
...
10. **市役所の扱い** — 組合申請の窓口を既存の市役所リソースに相乗りさせるか、
    独自メニューを立てるか。RP 上の運用（窓口 NPC を置くか、Discord 申請か）も含めて要決定
11. **承認できる権限者** — `/guild approve` を誰が撃てるか（ACE 権限 / 特定の job / 特定の identifier）
12. **銀行残高の所在** — 現金と銀行が別テーブル・別リソースの場合、センサス（§6.4）が
    どこまで読めるか。読めない財布があると総額の drift が常に大きく出てしまう
13. **想定同時接続数** — 委託所の注文数とキャッシュ戦略に影響（16 人規模ならキャッシュは単純な in-memory で足りる）
