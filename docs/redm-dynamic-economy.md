...
**実装との対応**（上の図は初期の設計スケッチで、実装後にいくつか名前・構成が変わっている）:
`dyn_economy_bridge` は VORP アダプタのみ実装（`server/vorp.lua` + 新規キャラ用
`server/newchar.lua`。RSGCore/RedEM:RP は未実装）。`dyn_market`（委託所）は
`resources/dyn_trade`（個人間の直接交渉取引）として実装。`dyn_guilds` は
`resources/dyn_guild` として実装。図に無い `resources/dyn_admin`（管理者向け
NUI ダッシュボード）が追加されている。
...
**実装状況**（全フェーズ実装済み。§6.5〜§6.7・§7・§9.3〜9.5 は既定 OFF）:
...
- Phase 3 … `resources/dyn_economy_bridge`（[README](../resources/dyn_economy_bridge/README.md)）と
  `resources/dyn_shop`。VORP アダプタ・決済フロー・NPC 店舗（露店の物売り NPC 付き）
- Phase 4 … `dyn_economy` の `server/recipes_import.lua`。`vorp_crafting` からの取り込み
- Phase 5.5〜9 … `dyn_economy` の `server/calibration.lua` / `server/playtime.lua`。
  bootstrap 較正のドライラン（§6.2）、新規キャラの開始所持金（§6.3、`dyn_economy_bridge`
  の `server/newchar.lua` が適用）、所持金分布への追従（§6.5）、収集効率アンカー（§6.6）、
  マネーサプライ PI 制御（§6.7）。全レイヤーを `/dyn_toggle` で個別にオン/オフできる
- Phase 5.6 ✅ … `dyn_economy` の `shared/census_math.lua` と `server/census.lua`。
  資産センサスと総額の健全性チェック（§6.4）
- Phase 5.7 ✅ … `resources/dyn_treasury`。税の記帳と国庫（§9.1/9.2）。州別・週次の
  税収集計にも対応（`state`/`period` 列。RedM が州で分かれているため一つの税に丸めない）
- Phase 6（相当） … `resources/dyn_trade`。委託所（§8）そのものではなく、個人間の
  直接交渉取引として同じ目的（P2P取引の記録・可視化）を果たす。実勢 VWAP を
  `dyn_economy`・`dyn_shop` の UI に表示し、NPC に売る損を可視化する（§8.2 相当）
- Phase 10〜11 ✅ … `dyn_economy` の `shared/progression_math.lua` / `server/progression.lua`。
  目標成長曲線ターゲティング（§7）。`/dyn_progress` で診断できる
- Phase 12 ✅ … `resources/dyn_guild`。組合の自己申告→運営承認、実績払い補助金。
  州ごと・週ごとの `dyn_treasury` 純税収を原資にする
- 管理者ダッシュボード … `resources/dyn_admin`。価格・在庫・税収・組合承認を NUI で一覧
...
| Phase | 内容 | 完了条件 | 状態 |
|---|---|---|---|
| 1 | スキーマ作成、`dyn_items` の初期投入 | テーブルが作られ、全取扱品に行がある | ✅ |
| 2 | `pricing.lua`（§4）＋ exports（§10） | `/dyn_quote <item> <qty>` で価格が返る | ✅ |
| 3 | ブリッジと NPC 店舗（`dyn_shop`） | 売買が動的価格で通り `dyn_npc_tx` に記録される | ✅ |
| 4 | レシピインポータと原価計算（§5） | `/dyn_import_recipes --apply` で下限価格が効く | ✅ |
| 5.5 | §6.2 の bootstrap 較正とドライラン | `/dyn_calibrate --dry-run` が現在値を出す | ✅ |
| 5.6 | 起動時の資産センサスと総額健全性チェック（§6.4） | 起動ログに中央値・外れ値・drift が出る | ✅ |
| 5.7 | 税の記帳と国庫（§9.1 / §9.2、州別週次） | `/treasury` で内訳が見える | ✅ |
| 6 | プレイヤー間取引の記録（§8 相当。委託所ではなく直接交渉） | `dyn_p2p_tx` に記録され、実勢 VWAP が見える | ✅ |
| 7〜9 | 日次スナップショット・収集効率アンカー・購買力追従・マネーサプライ PI（§6.5〜6.7） | 高効率アイテムの `price_index` が自動で動く | ✅（既定 OFF） |
| 10〜11 | `dyn_player_progress` の集計と目標成長曲線の適用（§7） | 重み付き誤差が不感帯に収束 | ✅（既定 OFF） |
| 12 | 組合の申請・承認と実績払い補助金（§9.3〜9.5） | 承認された組合に週次で補助が振り込まれる | ✅（既定 OFF） |
| 13 | 実データを見ながらパラメータ調整（§11） | 単一作物への集中が実測で緩和 | 運用しながら継続 |

**較正（Phase 7 以降）は必ず実測データが溜まってから有効化すること**。導入直後に
有効にすると、標本の薄い初期ログだけを見て価格が大きく歪む。§6.9 の安全装置
（キルスイッチ・ドライラン・ピン留め・標本下限）はすべて入っているが、
「いつ有効にするか」の判断は運営に委ねられている。
Phase 10〜11 の目標成長曲線は最も介入が強いので、Phase 7〜9 が安定してから最後に入れる。
Phase 5.7（税の記帳）は既定 ON。挙動を変えずに「どれだけ税が集まるのか」の実測が
取れるので、Phase 12（組合補助金）の予算はその数字を見てから決めるとよい。
...
6. ~~**プレイ時間の取得手段**~~ — VORP は保持していないため、`playtime.lua` が
   5 分おきに接続者へ加算する自前計測にした（クラッシュで多少の誤差が出ても事故で大きく狂わない設計）
7. **全キャラの所持金を集計できるか** — 銀行残高が別テーブル・別リソースの場合、
   §6.5 のマネーサプライ計算にどこまで含めるかを決める（現金のみ / 現金＋銀行 / ＋資産）
8. ~~**`config/items.lua` の中身**~~ — 実サーバーの `items` テーブルと突き合わせ、
   実在する156品目に差し替え済み。`dyn_shop/config.lua` の**座標はまだ仮**
   （実店舗の場所が決まり次第、差し替えが必要）
...
12. ~~**承認できる権限者**~~ — 他の管理コマンドと同じ ACE `dyn_economy.admin`
    に統一（`/guild approve` も `dyn_admin` ダッシュボードのボタンも同じ権限を見る）
