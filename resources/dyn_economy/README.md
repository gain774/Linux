...
| 実装済み（続き） | 自動較正（§6.2〜6.7 全部）: 後乗せ較正のドライラン、新規キャラの開始所持金、所持金分布への追従、収集効率アンカー、マネーサプライ PI 制御。個人間取引の参考価格（§8.2）。全レイヤーを個別にオン/オフできる（§6.9・§7.7） |
| 実装済み（続き） | 目標成長曲線ターゲティング（§7。**既定 OFF**）: log-log 補間、在籍度コホート、income_mult 補正、到達可能性(coverage)チェック、`/dyn_progress` 診断コマンド |
| 未実装 | 委託所（§8。dyn_trade が P2P 直接取引としてこの役割の大部分を代替している） |
...
/dyn_calibrate --dry-run              自動較正の現在値・トグル状態・センサス保留状態を表示
/dyn_toggle <key> <on|off> | list     需給変動・較正の各レイヤーを個別にオン/オフ（§6.9・§7.7）
/dyn_setfixed <item> <on|off>         品目を固定価格/変動価格に切り替える
/dyn_progress [--apply]               目標成長曲線の評価履歴を表示 / その場で評価を実行（§7）
/dyn_reload                           config と DB を読み直す
```

### 自動較正（§6〜§6.9）

`Config.Calibration.enabled = false` で§6.5〜6.7を全部止められるキルスイッチ。
`bootstrap` 以外は既定 OFF（`/dyn_toggle list` で今の状態を確認できる）。
資産センサス（§6.4）が異常を検知して保留中（`held`）の間は、§6.5〜6.7 の
長期較正だけが止まる（§4 の需給変動は止まらない）。

`Config.Economy` で新規キャラの開始所持金を決める（`vorp_NewCharacter` を
`dyn_economy_bridge` が拾って適用する。既存キャラには一切触れない）。
